class Todo:
    ...
    def get_by_priority(self, n):
        return [case for case, priority in self.things if priority == n]

    def get_low_priority(self):
        return [
            case
            for case, priority in self.things
            if priority == min(self.things, key=lambda x: x[1], default=0)[1]
        ]
    ...
    def get_high_priority(self):
        return [
            case
            for case, priority in self.things
            if priority == max(self.things, key=lambda x: x[1], default=5)[1]
        ]
