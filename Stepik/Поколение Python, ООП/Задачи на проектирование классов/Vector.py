...
class Vector:
    @staticmethod
    def _is_correct(first, second):
        if not isinstance(second, first.__class__):
            raise TypeError("Операнд должен быть вектором")
    ...
    def __add__(self, other):
        self._is_correct(self, other)
        return self.__class__(*map(sum, zip(self.coordinates, other.coordinates)))
